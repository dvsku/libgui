...
void backend_context_glfw::set_settings(const window_settings& settings) {
...
void backend_context_glfw::event_poll() {
    glfwPollEvents();
...
void backend_context_glfw::event_wait() {
    glfwWaitEvents();
}

void backend_context_glfw::set_tb_state(taskbar_state state) {
...
void backend_context_glfw::set_tb_value(uint64_t value) {
    if (!m_glfw_handle || !m_tb_handle) {
        return;
    }
    ...
    m_tb_handle->SetProgressValue(win32_handle, std::max<ULONGLONG>(value, 100), 100);
}

bool backend_context_glfw::is_closing() const {
    if (!m_glfw_handle) {
        return true;
    }
...
libgui::result backend_context_glfw::initialize(const window_settings& settings,
...
void backend_context_glfw::teardown() {
...
HWND backend_context_glfw::get_win32_handle() {
    if (!m_glfw_handle) {
        return nullptr;
    }
...
libgui::result backend_context_glfw::internal_initialize_glfw() {
    ...
    glfwSetWindowSizeCallback(m_glfw_handle, backend_context_glfw::internal_glfw_window_resize_callback);
    glfwSetFramebufferSizeCallback(m_glfw_handle, backend_context_glfw::internal_glfw_frame_buffer_resize_callback);
    glfwSetScrollCallback(m_glfw_handle, backend_context_glfw::internal_glfw_scroll_callback);
    glfwSetMouseButtonCallback(m_glfw_handle, backend_context_glfw::internal_glfw_mouse_btn_callback);
    glfwSetCursorPosCallback(m_glfw_handle, backend_context_glfw::internal_glfw_mouse_move_callback);
    glfwSetDropCallback(m_glfw_handle, backend_context_glfw::internal_glfw_drop_callback);

    result.success = true;
    return result;
}

void backend_context_glfw::internal_teardown_glfw() {

}

libgui::result backend_context_glfw::internal_initialize_win32() {
    libgui::result result;
    ...
    SetWindowLongPtr(win32_handle, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(backend_context_glfw::internal_wndproc_callback));

    result.success = true;
    return result;
}

void backend_context_glfw::internal_teardown_win32() {

}

void backend_context_glfw::internal_initialize_event_handlers() {
    internal_event_attach<internals::ev::ev_close>([this](const auto& event) {
    ...
    internal_event_attach<libgui::ev::ev_minimize_to_tb>([this](const auto& event) {
        internal_event_callback(event);
    });

    internal_event_attach<libgui::ev::ev_minimize_to_st>([this](const auto& event) {
        internal_event_callback(event);
    });
    ...
    internal_event_attach<libgui::ev::ev_minimize_restore_from_tb>([this](const auto& event) {
        internal_event_callback(event);
    });

    internal_event_attach<libgui::ev::ev_minimize_restore_from_st>([this](const auto& event) {
        internal_event_callback(event);
    });

    internal_event_attach<internals::ev::ev_update_settings>([this](const auto& event) {
        internal_event_callback(event);
    });

    internal_event_attach<internals::ev::ev_update_startup_settings>([this](const auto& event) {
        internal_event_callback(event);
    });
}

void backend_context_glfw::internal_teardown_event_handlers() {

}

void backend_context_glfw::internal_initialize_tb() {
...
void backend_context_glfw::internal_teardown_tb() {
    if (!m_tb_handle) {
        return;
    }
...
void backend_context_glfw::internal_initialize_st() {
    ...
    strncpy(m_st_handle->szTip, m_settings.title.c_str(),
        std::min<size_t>(sizeof(m_st_handle->szTip), m_settings.title.size()));
}

void backend_context_glfw::internal_teardown_st() {
    if (!m_st_handle) {
        return;
    }
...
bool backend_context_glfw::internal_set_st_icon_visible(bool value) {
    if (!m_st_handle)
        return false;
...
LRESULT __stdcall backend_context_glfw::internal_wndproc_callback(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
{
    backend_context_glfw* context = (backend_context_glfw*)GetWindowLongPtr(handle, GWLP_USERDATA);
...
LRESULT backend_context_glfw::internal_wndproc_wmsyscommand(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
{
...
LRESULT backend_context_glfw::internal_wndproc_wmsize(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
{
...
LRESULT backend_context_glfw::internal_wndproc_wmshowwindow(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
{
...
LRESULT backend_context_glfw::internal_wndproc_wmusertrayicon(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
{
...
LRESULT backend_context_glfw::internal_wndproc_wmnccalcsize(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
{
    if (!wparam) {
        return 0;
    }
...
LRESULT backend_context_glfw::internal_wndproc_wmnchittest(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
{
...
LRESULT backend_context_glfw::internal_wndproc_wmgetminmaxinfo(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
{
    // Get default values
...
LRESULT backend_context_glfw::internal_wndproc_wmnclbuttondown(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
{
    auto& flags = m_wnd->tb_flags;
...
LRESULT backend_context_glfw::internal_wndproc_wmnclbuttonup(HWND handle, UINT msg, WPARAM wparam, LPARAM lparam)
{
    auto& flags = m_wnd->tb_flags;
...
void backend_context_glfw::internal_event_callback(const internals::ev::ev_close& event) {
    if (!m_glfw_handle) {
        return;
    }
...
void backend_context_glfw::internal_event_callback(const libgui::ev::ev_close& event) {
    if (!m_glfw_handle) {
        return;
    }
...
void backend_context_glfw::internal_event_callback(const libgui::ev::ev_maximize& event) {
    if (!m_glfw_handle) {
        return;
    }
...
void backend_context_glfw::internal_event_callback(const libgui::ev::ev_minimize_to_tb& event) {
    if (!m_glfw_handle) {
        return;
    }
...
void backend_context_glfw::internal_event_callback(const libgui::ev::ev_minimize_to_st& event) {
    if (!m_glfw_handle) {
        return;
    }
...
void backend_context_glfw::internal_event_callback(const libgui::ev::ev_maximize_restore& event) {
    if (!m_glfw_handle) {
        return;
    }
...
void backend_context_glfw::internal_event_callback(const libgui::ev::ev_minimize_restore_from_tb& event) {
    if (!m_glfw_handle) {
        return;
    }
...
void backend_context_glfw::internal_event_callback(const libgui::ev::ev_minimize_restore_from_st& event) {
    if (!m_glfw_handle) {
        return;
    }
...
void backend_context_glfw::internal_event_callback(const internals::ev::ev_update_settings& event) {
...
void backend_context_glfw::internal_event_callback(const internals::ev::ev_update_startup_settings& event) {
    auto win32_handle = get_win32_handle();
    if (!win32_handle) {
        return;
    }
    ...
    if (minimum_size.width > 0) {
        ...
        size.width = std::max<int32_t>(size.width, minimum_size.width);
    }
    ...
    if (minimum_size.height > 0) {
        ...
        size.height = std::max<int32_t>(size.height, minimum_size.height);
    }
...
void backend_context_glfw::internal_glfw_window_resize_callback(GLFWwindow* window, int width, int height)
{
    backend_context_glfw* context = static_cast<backend_context_glfw*>(glfwGetWindowUserPointer(window));
    if (!context) {
        return;
    }
...
void backend_context_glfw::internal_glfw_frame_buffer_resize_callback(GLFWwindow* window, int width, int height)
{
    backend_context_glfw* context = static_cast<backend_context_glfw*>(glfwGetWindowUserPointer(window));
    if (!context) {
        return;
...
void backend_context_glfw::internal_glfw_scroll_callback(GLFWwindow* window, double dx, double dy)
{
    backend_context_glfw* context = static_cast<backend_context_glfw*>(glfwGetWindowUserPointer(window));
    if (!context) {
        return;
...
void backend_context_glfw::internal_glfw_mouse_btn_callback(GLFWwindow* window, int btn, int state, int mod)
{
    backend_context_glfw* context = static_cast<backend_context_glfw*>(glfwGetWindowUserPointer(window));
    if (!context) {
        return;
...
void backend_context_glfw::internal_glfw_mouse_move_callback(GLFWwindow* window, double x, double y)
{
    backend_context_glfw* context = static_cast<backend_context_glfw*>(glfwGetWindowUserPointer(window));
    if (!context) {
        return;
...
void backend_context_glfw::internal_glfw_drop_callback(GLFWwindow* window, int count, const char* files[])
{
    backend_context_glfw* context = static_cast<backend_context_glfw*>(glfwGetWindowUserPointer(window));
    if (!context) {
        return;
