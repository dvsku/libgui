...
libgui::result backend_context_glfw_opengl2::initialize(const window_settings& settings,
...
void backend_context_glfw_opengl2::teardown() {
    internal_teardown_event_handlers();
    backend_context_glfw::teardown();
}

void backend_context_glfw_opengl2::prepare_for_drawing() {

}

void backend_context_glfw_opengl2::draw() {
    glfwSwapBuffers(m_glfw_handle);
}

void backend_context_glfw_opengl2::clear(float r, float g, float b) {
...
void backend_context_glfw_opengl2::internal_initialize_event_handlers() {
    internal_event_attach<internals::ev::ev_frame_buffer_resized>([this](const auto& event) {
        internal_event_callback(event);
    });
}

void backend_context_glfw_opengl2::internal_teardown_event_handlers() {
...
void backend_context_glfw_opengl2::internal_event_callback(const internals::ev::ev_frame_buffer_resized& event)
{
