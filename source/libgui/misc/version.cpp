...
dv_version::dv_version(uint64_t packed) {
    m_packed_ver = packed;
    m_str_ver    = internal_packed_to_string(m_packed_ver);
}

dv_version::dv_version(uint16_t major, uint16_t minor, uint16_t rev) {
...
dv_version::dv_version(const std::string& major, const std::string& minor, const std::string& rev) {
...
uint64_t dv_version::get_packed() const {
    return m_packed_ver;
}

uint16_t dv_version::get_major() const {
    return static_cast<uint16_t>((m_packed_ver >> 48) & 0xFFFF);
}

uint16_t dv_version::get_minor() const {
    return static_cast<uint16_t>((m_packed_ver >> 32) & 0xFFFF);
}

uint16_t dv_version::get_rev() const {
    return static_cast<uint16_t>((m_packed_ver >> 16) & 0xFFFF);
}

const std::string& dvsku::dv_version::to_string() const {
    return m_str_ver;
}
